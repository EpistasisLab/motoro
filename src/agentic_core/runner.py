...
from agentic_core.models.run import AgentRun, RunStatus, RunStep
...
async def init_schema(*, drop_first: bool = False) -> None:
    ...
    ``python -m agentic_core.migrations upgrade``, run as a deploy step. This
...
def _session(reason: str) -> AbstractAsyncContextManager[AsyncSession]:
    ...
    return system_session(reason=f"agentic_core.runner: {reason}")
