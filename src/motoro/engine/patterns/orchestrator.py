...
from motoro.engine.patterns import catalog, composition
...
from motoro.schemas.llm import ActOutput, PlanOutput, ReasonOutput, SenseOutput
...
async def _update_heartbeat(db: Any, run_id: uuid.UUID) -> None:
    ...
    from motoro.worker.resilience import db_retry
...
class PatternOrchestrator:
    ...
    async def run(
        ...
        try:
                    ...
                    for phase_idx in range(first_phase, len(phase_sequence)):
                        ...
                        if phase_name == "plan":
                            ...
                            if isinstance(act_output, ActOutput):
                                ...
                                if wm is not None:
                                    from motoro.memory.working import WorkingMemoryEntryType
            ...
            from motoro.observability.metrics import record_error
        ...
        # --- Episodic memory ------------------------------------------------
        from motoro.observability.metrics import record_run
    ...
    def from_pattern_config(
        ...
        *default_execution_pattern*. Callers that know the run's model should
        ...
        see ``motoro.services.llm_service.model_supports_tool_calling``.
        """
