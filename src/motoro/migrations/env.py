...
product that ``pip install``s motoro has to be able to run this chain. A
script directory outside the wheel would not ship.

Two properties make this chain safe to run alongside a product's own:

* ``version_table`` is ``alembic_version_motoro``, not ``alembic_version``,
...
import motoro.models.agent  # noqa: F401
import motoro.models.mcp_server  # noqa: F401
import motoro.models.memory  # noqa: F401
...
if context.is_offline_mode():
    ...
    # use ``motoro.migrations.upgrade`` instead of invoking alembic directly.
