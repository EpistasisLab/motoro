...
``from motoro.config import settings`` / ``settings.field`` call pattern
...
def configure(settings_instance: CoreSettings) -> None:
    ...
    if _accessed and _instance is not None and settings_instance is not _instance:
        raise RuntimeError(
            "motoro.config.configure() called after settings were already read. "
...
class _SettingsProxy:
    ...
    Exists so that ``from motoro.config import settings`` can be imported
